#!/bin/sh
# verify 一次性服务入口：代码测试 → 构建检查 → 规定场景断言 → HTTP 冒烟 → 共享许可断言。
...
echo '== [5/5] 共享许可组合断言（HTTP 冒烟成功后，确保不误报超限） =='
npx esbuild scripts/verify-permission.ts \
  --bundle --platform=node --format=esm --outfile=.verify/permission.mjs
node .verify/permission.mjs
